...
const excluded=rel=>rel.includes('/.pytest_cache/')||rel.startsWith('.pytest_cache/');
...
let owned=0, generated=0;
for(const file of walk(root)){
  const rel=posix(path.relative(root,file));
  const text=fs.readFileSync(file,'utf8');
  if(excluded(rel)){generated++; continue;}
  owned++;
...
console.log(`Documentation conformance OK: ${owned} PowerFarm Markdown files mapped; ${generated} generated files exempt.`);
